...
final class VerifiedSpriteCatalogTests: XCTestCase {
    func testCatalogContainsBundledPixelAndClawSprites() {
        ...
        XCTAssertEqual(Set(catalog.availableSpecies(for: .claw)), Set(Species.allCases))
        ...
        for species in Species.allCases {
            ...
            XCTAssertTrue(catalog.isAvailable(style: .claw, species: species))
            XCTAssertNotNil(catalog.spriteURL(for: species, style: .pixel))
            XCTAssertNotNil(catalog.spriteURL(for: species, style: .claw))
            XCTAssertEqual(catalog.resolvedStyle(preferred: .claw, species: species), .claw)
        }
    }
}
